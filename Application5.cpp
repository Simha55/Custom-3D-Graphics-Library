...
int Application5::Initialize()
{
	...
	GzToken     nameListOffset[5];		/* light info */
	GzPointer   valueListOffset[5];
	...
	/* Translation matrix */
		...
		*/
		...
		if (i == AAKERNEL_SIZE) {
			valueListOffset[0] = (GzPointer)&baseOffset;
			valueListOffset[1] = (GzPointer)&baseOffset;
		}
		else {
...
int Application5::Render() 
{
	...
	for (int i = 0; i < AAKERNEL_SIZE; i++) {
		...
		*/
		...
		//m_pRender[i]->GzFlushDisplay2File(outfile); 	/* write out or update display to file*/
	...
	for (int j = 0; j < m_nWidth * m_nHeight; j++) {
		...
		float b = 0;
		for (int i = 0; i < AAKERNEL_SIZE; i++) {
			r += (float)m_pRender[i]->pixelbuffer[j].red * AAFilter[i][2];
			g += (float)m_pRender[i]->pixelbuffer[j].green * AAFilter[i][2];
			b += (float)m_pRender[i]->pixelbuffer[j].blue * AAFilter[i][2];
		}

		m_pRender[0]->pixelbuffer[j].red = (short)(r);
		m_pRender[0]->pixelbuffer[j].green = (short)(g);
		m_pRender[0]->pixelbuffer[j].blue = (short)(b);
	}
	...
	m_pRender[0]->GzFlushDisplay2File(outfile);
	m_pRender[0]->GzFlushDisplay2FrameBuffer();
